...
async function updateFlight(
  ...
  const tx = await agency._applyUpdate(
    proofObject,
    insurance_flight_id
   );
  await tx.wait();
...
export async function checkFlightDelayAction (aircraft_icao: string, flight_no: string, insurance_flight_id: number, walletProvider: any) {
  try {
      ...
      const proof:any = await retrieveDataAndProof(data.abiEncodedRequest, roundId);
      try {
          const result = await updateFlight(insuredFlightsAgencyContract, insurance_flight_id, proof);
          return result;
      } catch (error) {
          console.log("error", error)
          return error;
